...
def get_resnet(input_image,stage5=False, train_bn=True):
    ...
    x = Conv2D(64, (7, 7), strides=(2, 2), name='conv1', use_bias=True)(x)#第一层卷积，压缩图片为原来的一半
    x = BatchNormalization(name='bn_conv1')(x, training=train_bn)
    x = Activation('relu')(x)
    # Height/4,Width/4,64
    C1 = x = MaxPooling2D((3, 3), strides=(2, 2), padding="same")(x)#最大池化后再压缩一半

    # Stage 2
    x = conv_block(x, 3, [64, 64, 256], stage=2, block='a', strides=(1, 1), train_bn=train_bn)#步长为1，长宽不变
