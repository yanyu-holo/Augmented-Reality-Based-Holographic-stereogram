    ...
    for i in range(0, len(count)):
        ...
        if os.path.isfile(path) and path.endswith('json'):
            ...
            if not os.path.exists("TRAINDATA"):
                os.mkdir("TRAINDATA")
            label_path = "TRAINDATA/mask"
            if not os.path.exists(label_path):
                os.mkdir(label_path)
            img_path = "TRAINDATA/imgs"
            if not os.path.exists(img_path):
                os.mkdir(img_path)
            yaml_path = "TRAINDATA/yaml"
            if not os.path.exists(yaml_path):
                os.mkdir(yaml_path)
            label_viz_path = "TRAINDATA/label_viz"
            ...
            #PIL.Image.fromarray(lbl).save(osp.join(label_path, str(index)+'.png'))
